...
function get_fir_operand(){
  let first_oper = document.getElementById('myinput2').value;
  let res = document.getElementById('myoutput');
  res.style.color="#66FCF1";
  first_oper = first_oper.trim();
  let bash = /\.\.|\*\*\*|\/\/|\+\+|%%|\-\-/g;
  if(bash.test(first_oper)){
    res.style.color="red";
    res.value="!Invalid input";
    return;
  }
  if((first_oper != "")){
    let expression1 = first_oper.replace(/ +/g, "");
    expression1 = expression1.replace(/\.\./g,".");
    first_oper = expression1;
    let hregx = /[g-zG-Z]/g;
    if(hregx.test(expression1)){
      res.style.color="red";
      res.value="!Invalid input";
    }else{
        hregx = /([\.0-9a-fA-F]+)/g;
        expression1 = expression1.replace(hregx,(matchs)=>{
                      fir_result="";
                      fir_result2="";
                      if(matchs.indexOf(".") != -1) fir_result2 = HexToBin(matchs);
                      if(!isNaN(parseInt(matchs,16)))
                        fir_result=parseInt(matchs,16).toString(2);
                      return (fir_result.toString().toUpperCase()+fir_result2); });
        first_operand = parseFloat((fir_result.toString().toUpperCase()+fir_result2), 2);
        return first_operand;
    }  
  }
}

function get_sec_operand(){
  let second_oper = document.getElementById('myinput3').value;
  let res = document.getElementById('myoutput');
  res.style.color="#66FCF1";
  second_oper = second_oper.trim();
  let bash = /\.\.|\*\*\*|\/\/|\+\+|%%|\-\-/g;
  if(bash.test(second_oper)){
    res.style.color="red";
    res.value="!Invalid input";
    return;
  }
  if((second_oper != "")){
    let expression2 = second_oper.replace(/ +/g, "");
    expression2 = expression2.replace(/\.\./g,".");
    second_oper = expression2;
    let hregx = /[g-zG-Z]/g;
    if(hregx.test(expression2)){
      res.style.color="red";
      res.value="!Invalid input";
    }else{
        hregx = /([\.0-9a-fA-F]+)/g;
        expression2 = expression2.replace(hregx,(matchs)=>{
                      sec_result="";
                      sec_result2="";
                      if(matchs.indexOf(".") != -1) sec_result2 = HexToBin(matchs);
                      if(!isNaN(parseInt(matchs,16)))
                        sec_result=parseInt(matchs,16).toString(2);

                      return (sec_result.toString().toUpperCase()+sec_result2); });
                      second_operand = parseFloat((sec_result.toString().toUpperCase()+sec_result2), 2);
...
function get_base(){
  let base = document.getElementById('myinput1').value;
  let res = document.getElementById('myoutput');
  res.style.color="#66FCF1";
  base = base.trim();
  let bash = /\.\.|\*\*\*|\/\/|\+\+|%%|\-\-/g;
  ...
  if((base != "")){
    let expression1 = base.replace(/ +/g, "");
    expression1 = expression1.replace(/\.\./g,".");
    base = expression1;
    let hregx = /[g-zG-Z]/g;
    if(hregx.test(expression1)){
      ...
      res.value="!Invalid input";
        ...
        expression1 = expression1.replace(hregx,(matchs)=>{
          base="";
          base2="";
                      if(matchs.indexOf(".") != -1) base2 = HexToBin(matchs);
                      if(!isNaN(parseInt(matchs,16)))
                      base=parseInt(matchs,16).toString(2);
                      return (base.toString().toUpperCase()+base2); });
        base_polynom = parseFloat((base.toString().toUpperCase()+base2), 2);
        return base_polynom;
    }  
  }
}
...
function multiply(){
  let res = document.getElementById('myoutput');
  res.style.color="#66FCF1";
  firOper = get_fir_operand();
  secOper = get_sec_operand();
  base_polynom = get_base();
  var Ans = 0;
  temp_num = parseInt(secOper.toString(), 10);
  length_secOper = secOper.toString().length;
  for (let i = 0; i < length_secOper; i++){
    last_bit = temp_num % 10;
    temp_num = Math.floor(temp_num/10);
    if(last_bit == 0){
      ans = 0;
    }
    else{
      ans = bin2dec(firOper) << i;
    }
    Ans = Ans ^ ans; 
  }
  Ans = dec2bin(Ans);

  //find remainder of Ans by base_polynom
  length_Ans = Ans.toString().length;
  length_base = base_polynom.toString().length;
  while(length_Ans >= length_base){
    Ans = bin2dec(Ans);
    Ans = Ans ^ (bin2dec(base_polynom) << (length_Ans-length_base));
    Ans = dec2bin(Ans);
    length_Ans = Ans.toString().length;
    length_base = base_polynom.toString().length;
  }
  Ans_inHex = parseInt(Ans, 2).toString(16).toUpperCase();
  complete_ans = Ans.toString(2)+"\n"+"("+Ans_inHex+")"+"h";
  res.value = complete_ans;
}
...
function plus(){
  ...
  base_polynom = get_base();
  var Ans = 0;
  Ans = bin2dec(firOper) ^ bin2dec(secOper);
  Ans = dec2bin(Ans);
  length_Ans = Ans.toString().length;
  length_base = base_polynom.toString().length;
  while(length_Ans >= length_base){
